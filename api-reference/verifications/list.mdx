...
```bash
curl 'https://clear-api.nfigate.com/api/v1/verifications?status=approved&sortBy=resolvedAt&sortOrder=desc' \
  -H 'X-API-Key: nfi_your_api_key_here'
```
...
async function getAllVerifications() {
  ...
  while (hasMore) {
    const response = await fetch(
      `https://clear-api.nfigate.com/api/v1/verifications?page=${page}&limit=100`,
      {
